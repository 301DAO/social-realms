 ...
 */
export const authenticateAndGetClient =
  async (): Promise<CeramicApi | null> => {
...
export const detectFollowListChange = async (
  ...
  forAddress: string,
  knownFollowingList: string[] | undefined | null,
  timeoutMs: number
): Promise<any> => {
  const timestamp = Date.now()
  const response: any = await loadFollowingForAddress(ceramicClient, forAddress)
